...
public class CollectionValueIteratorTest extends BlueDbDiskTestBase {
	...
	public void test_timeout() throws Exception {
		...
		try (CollectionValueIterator<TestValue> iterator =  new CollectionValueIterator<>(getTimeSegmentManager(), new Range(0, 3), false)) {
			iteratorContents = toList(iterator);
			iterator.close();
		...
		try (CollectionValueIterator<TestValue> iterator =  new CollectionValueIterator<>(getTimeSegmentManager(), new Range(0, 3), 20, false)) {
