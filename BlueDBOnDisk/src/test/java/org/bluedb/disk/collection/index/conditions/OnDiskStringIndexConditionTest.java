...
public class OnDiskStringIndexConditionTest {
	
	private static final String INDEX_NAME = "index-name";
	private static final Path INDEX_PATH = Paths.get(INDEX_NAME);
	
	private ReadableIndexOnDiskMocker<StringKey, TestValue> indexMocker = new ReadableIndexOnDiskMocker<>();
	private OnDiskStringIndexCondition<TestValue> indexCondition = new OnDiskStringIndexCondition<>(indexMocker.getIndex());
	...
	public void test_extractIndexValueFromKey() {
		...
		for(BlueEntity<TestValue> entity : allEntities) {
			...
			if(indexKey instanceof StringKey) {
				String expectedIndexValue = indexKey == null ? null : ((StringKey)indexKey).getId();
	...
	public void test_conditionConfiguration_exceptionsThrownForPassingNullIn() {
		...
		try {
			indexCondition.isIn((Set<String>)null);
			fail();
		} catch(InvalidParameterException e) { }
		
		try {
			indexCondition.isIn((BlueSimpleSet<String>)null);
			fail();
		} catch(InvalidParameterException e) { }
	...
	public void test_conditionConfiguration_exceptionsThrownForAddingConditionsAfterSettingEqualsCondition() {
		...
		try {
			indexCondition.isIn(new BlueSimpleInMemorySet<String>(new HashSet<>()));
			fail();
		...
		try {
			indexCondition.meets(indexedString -> false);
			fail();
		} catch(IllegalStateException e) { }
	}
	...
	public void test_meets() {
		HashSet<String> valuesToMatch = new HashSet<String>(Arrays.asList("10", "24", "29"));
	...
	private ValueKey toIndexKey(TestValue collectionValue) {
		if(collectionValue.getCupcakes() == Integer.MIN_VALUE) {
			...
			return new StringKey(String.valueOf(collectionValue.getCupcakes()));
		}
	}
